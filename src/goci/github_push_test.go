...
func performGithubRequest(json string, t *testing.T) {
	w := NewLoggingRW(t)
	...
	signalRegister <- not
	defer func() {
		signalUnregister <- not
	}()

	go func() {
		for {
			sig := <-not
			if ev, ex := sig["event"]; ex && ev == "processfinished" {
				found <- true
				return
			}
		}
...
func TestGOPATHRepo(t *testing.T) {
	performGithubRequest(`{"pusher":{"name":"zeebo","email":"leterip@me.com"},"repository":{"name":"bencode",
	...
	"2ddf0ab03e2d79162c08ce0ba59496a8ef250a91","created":false}`, t)
}

func TestCleanedUp(t *testing.T) {
	performGithubRequest(`{"pusher":{"name":"zeebo","email":"leterip@me.com"},"repository":{"name":"heroku-basic-app",
